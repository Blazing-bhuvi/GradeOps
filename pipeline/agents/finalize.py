...
from pipeline.server.db import get_sync_client
...
def finalize_agent(state: ExamGradingState) -> dict:
    ...
    try:
        ...
        # 1. Store submissions
        if gradebook:
            # Use upsert or delete/insert for reproducibility
            db.submissions.delete_many({"exam_id": exam_id})
            db.submissions.insert_many(gradebook)
        
        # 2. Update exam status and stats
        db.exams.update_one(
            {"id": exam_id},
            {
                "$set": {
                    "status": "graded",
                    "stats": stats,
                    "students": stats.get("total_students", len(gradebook)),
                    "reviewed": stats.get("total_students", len(gradebook)),
                    "finalized_at": stats["finalized_at"]
                }
            }
        )
        print(f"[finalize] Persisted to MongoDB and marked as graded: {exam_id}")
    except Exception as e:
        print(f"[finalize] Error persisting to MongoDB: {e}")
