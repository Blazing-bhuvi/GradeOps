...
export async function render(container) {
  ...
  const gradedCount   = exams.filter(e => e.status === 'graded' || e.status === 'complete').length;
  const pendingExams  = exams.filter(e => e.status === 'processing' || e.status === 'awaiting_review');
